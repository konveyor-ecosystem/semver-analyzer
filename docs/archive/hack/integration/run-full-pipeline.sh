...
if [[ "$SKIP_ANALYSIS" == false ]]; then
    ...
    if [[ -d "$PF_CSS_REPO/.git" ]]; then
        ...
        DEP_REPO_FLAG="--dep-repo $PF_CSS_REPO --dep-build-command 'yarn install && npx gulp buildPatternfly'"
        if [[ -n "$PF_CSS_TAG" ]]; then
            DEP_REPO_FLAG="$DEP_REPO_FLAG --dep-to $PF_CSS_TAG"
        fi
        if [[ -n "$PF_CSS_FROM" ]]; then
            DEP_REPO_FLAG="$DEP_REPO_FLAG --dep-from $PF_CSS_FROM"
        fi
